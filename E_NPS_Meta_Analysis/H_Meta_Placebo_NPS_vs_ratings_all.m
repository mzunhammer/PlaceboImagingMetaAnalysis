...
stats_NPS=withinMetastats(placebo(:),control(:));
stats_rating=withinMetastats(pla_rating(:),con_rating(:));
[r,~,r_ci_lo,r_ci_hi]=corrcoef(stats_NPS.std_delta,stats_rating.std_delta,'rows','complete');
corr(i)=r(2);
corr_ci_lo(i)=r_ci_lo(2);
corr_ci_hi(i)=r_ci_hi(2);
...
stats_NPS=withinMetastats(pla_NPS(:),con_NPS(:));
stats_rating=withinMetastats(pla_rating(:),con_rating(:));
[r,~,r_ci_lo,r_ci_hi]=corrcoef(stats_NPS.std_delta,stats_rating.std_delta,'rows','complete');
corr(i)=r(2);
corr_ci_lo(i)=r_ci_lo(2);
...
%Between-subject studies not included in correlation analysis
corr(i)=NaN;
corr_ci_lo(i)=NaN;
corr_ci_hi(i)=NaN;
...
% WARNING: only contrasts available for NPS-scores & ratings anyways
[r,~,r_ci_lo,r_ci_hi]=corrcoef(zscore(pla_effect),zscore(rating_diff),'rows','complete');
corr(i)=r(2);
corr_ci_lo(i)=r_ci_lo(2);
corr_ci_hi(i)=r_ci_hi(2);
...
stats_NPS=withinMetastats(placebo(:),control(:));
[r,~,r_ci_lo,r_ci_hi]=corrcoef(stats_NPS.std_delta,zscore(rating_diff),'rows','complete');
corr(i)=r(2);
corr_ci_lo(i)=r_ci_lo(2);
corr_ci_hi(i)=r_ci_hi(2);
...
responders=rating_diff<0; % INVERSE CONTRAST THAN OTHER STUDIES


i=find(strcmp(studies,'zeidan'));
% Contrasts only !!  CONTRAST INVERSE COMPARED TO OTHER STUDIES !!
[r,~,r_ci_lo,r_ci_hi]=corrcoef(pla_effect*-1,rating_diff*-1,'rows','complete');
corr(i)=r(2);
corr_ci_lo(i)=r_ci_lo(2);
corr_ci_hi(i)=r_ci_hi(2);
