...
studyIDtexts={
			...
			'Eippert et al. 2009: Control vs placebo cream (saline group)| heat (late)';...
        ...
        % correlation for ratings in all other studies, which was: 0.6654
...
pla_neg_rating=df((strcmp(df.studyID,'kessner')&strcmp(df.cond,'pain_placebo_neg')),'rating');
pla_pos_rating=df((strcmp(df.studyID,'kessner')&strcmp(df.cond,'pain_placebo_pos')),'rating');
...
stats(i)=betweenMetastats(pla_pos_rating{:,1},pla_neg_rating{:,1});
...
% Get stats for between-subject design
stats(i)=betweenMetastats(pla_rating,con_rating);
...
stats(i)=withinMetastats(-rating_diff{:,:},0.6451);%NO RAW RATINGS AVAILABLE, ONLY RATING CONTRAST CONTROL-PLACEBO
...
rating_diff=df((strcmp(df.studyID,'wager_princeton')&strcmp(df.cond,'(intense-none)control-placebo')),'rating'); %NO RAW RATINGS AVAILABLE, ONLY RATING CONTRAST CONTROL-PLACEBO
...
stats(i)=withinMetastats(-rating_diff{:,:},0.6451);%NO RATINGS AVAILABLE
...
% WARNING: The standardized effect can only be computed when using betas or
% non-differential cons
stats(i)=withinMetastats(rating_diff,0.6451);

%% Summarize all studies, weigh by SE
% Summary analysis+ Forest Plot
ForestPlotter(stats,studyIDtexts,'Rating difference (Hedge''s g)','random');

hgexport(gcf, 'Ratings_Meta_Basic.eps', hgexport('factorystyle'), 'Format', 'eps'); 
