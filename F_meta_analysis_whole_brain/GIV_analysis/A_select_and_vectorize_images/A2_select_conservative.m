...
% Atlas: None 
% Bingel06: None
% Bingel11: Excluded due to fixed testing sequence
% Choi: None
% Eippert: None
% Ellingsen: None
% Elsenbruch: None
...
% Geuter: None
% Kessner: None
% Kong06: Excluded bc of missing data
% Kong09: None
...
% Wrobel: None
% Zeidan: Excluded bc of missing data + different imaging modality (ASL)
...
df=df(~df.ex_all,:); % Exclude outlier participants

df=df(~strcmp(df.studyID,'bingel11'),:);
df=df(~strcmp(df.studyID,'kong06'),:);
df=df(~strcmp(df.studyID,'ruetgen'),:);
df=df(~strcmp(df.studyID,'wager04b_michigan'),:);
df=df(~strcmp(df.studyID,'zeidan'),:);

studies=unique(df.studyID);   %Get all remaining studies in df
...
%% Study-level data
df_conserv.variables='Normalized Images';

df_conserv.studies=studies;
%Between-subject studies
df_conserv.BetweenSubject=zeros(size(studies));
df_conserv.BetweenSubject(strcmp(studies,'kessner'))=1;
...
%...for imaging data
df_conserv.consOnlyImg=zeros(size(studies));
df_conserv.consOnlyImg(strcmp(studies,'wager04a_princeton'))=1;
%Studies where only contrasts between conditions (e.g. pla>con), rather than conditions themselves (pla pain & con pain separately) are available
%...for rating data
df_conserv.consOnlyRating=zeros(size(studies));
df_conserv.consOnlyRating(strcmp(studies,'wager04a_princeton'))=1;
...
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'StimHiPain_Open_Stimulation'),'norm_img'})+... %If norm_img not in df, run B_Prepro_Images/A_Equalize_Image_Size_and_Mask first.
...
df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'StimHiPain_Hidden_Stimulation'),'norm_img'})+...
...
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'StimHiPain_Open_Stimulation'),'rating'}+... %If norm_img not in df, run B_Prepro_Images/A_Equalize_Image_Size_and_Mask first.
                       c_df{strcmp(c_df.cond,'StimHiPain_Open_RemiConz'),'rating'}+...
                       c_df{strcmp(c_df.cond,'StimHiPain_Open_ExpectationPeriod'),'rating'};
df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'StimHiPain_Hidden_Stimulation'),'rating'}+...
...
df_conserv.pla_img{i}= nanmean(cat(3,v_masked(placebo{:,'norm_img_placebo_R'}),...
                                  v_masked(placebo{:,'norm_img_placebo_L'})),3);
df_conserv.con_img{i}= nanmean(cat(3,v_masked(control{:,'norm_img_control_R'}),...
                                  v_masked(control{:,'norm_img_control_L'})),3);

df_conserv.pla_rating{i}= nanmean(cat(3,placebo{:,'rating_placebo_R'},...
                                     placebo{:,'rating_placebo_L'}),3);
df_conserv.con_rating{i}= nanmean(cat(3,control{:,'rating_control_R'},...
...
df_conserv.con_img{i}=           v_masked(c_df{~cellfun(@isempty,regexp(c_df.cond,'Exp1_control_pain_beta.*')),'norm_img'});
df_conserv.pla_img{i}=mean(cat(3,v_masked(c_df{~cellfun(@isempty,regexp(c_df.cond,'Exp1_100potent_pain_beta.*')),'norm_img'}),...
                              v_masked(c_df{~cellfun(@isempty,regexp(c_df.cond,'Exp1_1potent_pain_beta.*')),'norm_img'})),3);

df_conserv.con_rating{i}=           c_df{~cellfun(@isempty,regexp(c_df.cond,'Exp1_control_pain_beta.*')),'rating'};
df_conserv.pla_rating{i}=mean(cat(3,c_df{~cellfun(@isempty,regexp(c_df.cond,'Exp1_100potent_pain_beta.*')),'rating'},...
...
df_conserv.con_img{i}=[mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'pain early: control_saline'),'norm_img'}),...
...
df_conserv.pla_img{i}=[mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'pain early: placebo_saline'),'norm_img'}),...
...
df_conserv.con_rating{i}=[mean(cat(3,c_df{strcmp(c_df.cond,'pain early: control_saline'),'rating'},...
...
df_conserv.pla_rating{i}=[mean(cat(3,c_df{strcmp(c_df.cond,'pain early: placebo_saline'),'rating'},...
...
df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'Painful_touch_control'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'Painful_touch_placebo'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'Painful_touch_control'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'Painful_touch_placebo'),'rating'};

%'elsenbruch'
i=find(strcmp(studies,'elsenbruch'));
c_df=df(strcmp(df.studyID,'elsenbruch'),:);%current df

df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_control_0%_analgesia'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_placebo_100%_analgesia'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'pain_control_0%_analgesia'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'pain_placebo_100%_analgesia'),'rating'};

%'freeman'
i=find(strcmp(studies,'freeman'));
c_df=df(strcmp(df.studyID,'freeman'),:);%current df

df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_post_control_high_pain'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_post_placebo_high_pain'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'pain_post_control_high_pain'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'pain_post_placebo_high_pain'),'rating'};

%'geuter'
i=find(strcmp(studies,'geuter'));
c_df=df(strcmp(df.studyID,'geuter'),:);%current df

df_conserv.con_img{i}=mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'early_pain_control_weak'),'norm_img'}),...
...
df_conserv.pla_img{i}=mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'early_pain_placebo_weak'),'norm_img'}),...
...
df_conserv.con_rating{i}=mean(cat(3,c_df{strcmp(c_df.cond,'early_pain_control_weak'),'rating'},...
...
df_conserv.pla_rating{i}=mean(cat(3,c_df{strcmp(c_df.cond,'early_pain_placebo_weak'),'rating'},...
...
df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_placebo_neg'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_placebo_pos'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'pain_placebo_neg'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'pain_placebo_pos'),'rating'};

%'kong09'
i=find(strcmp(studies,'kong09'));
c_df=df(strcmp(df.studyID,'kong09'),:);%current df

df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_post_control'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_post_placebo'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'pain_post_control'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'pain_post_placebo'),'rating'};

%'lui'
i=find(strcmp(studies,'lui'));
c_df=df(strcmp(df.studyID,'lui'),:);%current df

df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_control'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'pain_placebo'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'pain_control'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'pain_placebo'),'rating'};

%'schenk'
i=find(strcmp(studies,'schenk'));
c_df=df(strcmp(df.studyID,'schenk'),:);%current df

df_conserv.con_img{i}=mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'pain_nolidocaine_control'),'norm_img'}),...
                              v_masked(c_df{strcmp(c_df.cond,'pain_lidocaine_control'),'norm_img'})),3);
df_conserv.pla_img{i}=mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'pain_nolidocaine_placebo'),'norm_img'}),...
                              v_masked(c_df{strcmp(c_df.cond,'pain_lidocaine_placebo'),'norm_img'})),3);

df_conserv.con_rating{i}=mean(cat(3,c_df{strcmp(c_df.cond,'pain_nolidocaine_control'),'rating'},...
                              c_df{strcmp(c_df.cond,'pain_lidocaine_control'),'rating'}),3);
df_conserv.pla_rating{i}=mean(cat(3,c_df{strcmp(c_df.cond,'pain_nolidocaine_placebo'),'rating'},...
...
df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'control_pain'),'norm_img'});
df_conserv.pla_img{i}=v_masked(c_df{strcmp(c_df.cond,'placebo_pain'),'norm_img'});

df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'control_pain'),'rating'};
df_conserv.pla_rating{i}=c_df{strcmp(c_df.cond,'placebo_pain'),'rating'};
...
df_conserv.pla_img{i}=(-1).*v_masked(c_df{strcmp(c_df.cond,'(intense-none)control-placebo'),'norm_img'});
df_conserv.con_img{i}=v_masked(c_df{strcmp(c_df.cond,'intense-none'),'norm_img'}); % BASELINE PAIN IS ADDED ADDED AS A CONTROL CONDITION HERE FOR PAIN vs BASELINE ANALYSIS, BUT NOT USED IN PLACEBO VS CONTROL ANALYSIS 
df_conserv.pla_rating{i}=(-1).*c_df{strcmp(c_df.cond,'(intense-none)control-placebo'),'rating'};
df_conserv.con_rating{i}=c_df{strcmp(c_df.cond,'intense-none'),'rating'}; % BASELINE PAIN IS ADDED ADDED AS A CONTROL CONDITION HERE FOR PAIN vs BASELINE ANALYSIS, BUT NOT USED IN PLACEBO VS CONTROL ANALYSIS

%'wrobel'
i=find(strcmp(studies,'wrobel'));
c_df=df(strcmp(df.studyID,'wrobel'),:);%current df

df_conserv.con_img{i}=[mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'early_pain_control_saline'),'norm_img'}),...
...
df_conserv.pla_img{i}=[mean(cat(3,v_masked(c_df{strcmp(c_df.cond,'early_pain_placebo_saline'),'norm_img'}),...
...
df_conserv.con_rating{i}=[mean(cat(3,c_df{strcmp(c_df.cond,'early_pain_control_saline'),'rating'},...
...
df_conserv.pla_rating{i}=[mean(cat(3,c_df{strcmp(c_df.cond,'early_pain_placebo_saline'),'rating'},...
