...
con_ID=[2;3;4;5;6];
    ...
    %Build Pain vector
        switch con_ID(i)
            ...
            nBlocks(j,i)=length(Sess{1,1}.ons{1,1})+...
            ...
            rating(j,i)=NaN;
                        nBlocks(j,i)=length(Sess{1,1}.ons{1,5})+...
...
wager_princeton.imgsPerBlock =zeros(size(cond)); % Everything modeled as events accroding to paper. However, each anticipation periods cosisted of many events
wager_princeton.nBlocks      =nBlocks; % Sum of events within each condition, according to SPM
wager_princeton.nImages      =zeros(size(cond)); % all events
