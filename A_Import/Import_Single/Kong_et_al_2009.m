...
kong09.plaFirst=ones(size(kong09.img)).*0.5; %pre- sentation of these four random sequences alternated between the radial and ulnar sides of the medial surface of the forearm (i.e. the placebo and the control sides)
...
kong09.imgsPerBlock =ones(size(img))*6; % stimulus length from paper (12 s / 2s TR), no SPM or other source available.
kong09.nBlocks      =[ones(size(img_pain_control_pre))*2*6;... % According to behavioral data file tis should be 2*6 identical stimuli per participants and condition
...
kong09.xSpan        =NaN(size(img)); % currently unknown(?)
kong09.conSpan      =NaN(size(img)); % currently unknown(?)
