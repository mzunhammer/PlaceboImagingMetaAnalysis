...
load('A2_Conservative_Sample.mat')
...
for j=1:length(varselect) % Loop through all outcome variables
    currvar=varselect{j};
    v=find(strcmp(df_conserv.variables,currvar));
    for i=1:length(df_conserv.studies) % Loop through all studies...
        rating_var_and_con_only=(any(strcmp(ratingvars,currvar)) && df_conserv.consOnlyRating(i)==1);
        img_var_and_con_only=(any(strcmp(imagingvars,currvar)) && df_conserv.consOnlyNPS(i)==1)
        if  ~rating_var_and_con_only && ~img_var_and_con_only % where both pla and con is available.
            if df_conserv.BetweenSubject(i)==0 %Use withinMetastats for within-subject studies
               stats.(currvar)(i)=withinMetastats(df_conserv.pladata{i}(:,v),df_conserv.condata{i}(:,v));
            elseif df_conserv.BetweenSubject(i)==1 %Use betweenMetastats for between-subject studies
               stats.(currvar)(i)=betweenMetastats(df_conserv.pladata{i}(:,v),df_conserv.condata{i}(:,v));
            end        
        end
    end
end
...
conOnly=find(df_conserv.consOnlyRating==1);
for j=1:length(ratingvars)
    ...
    for i=conOnly'
        stats.(currvar)(i)=withinMetastats(df_conserv.pladata{i}(:,v),impu_r);
    end
end

% Loop for studies with contrast-only imaging data
conOnly=find(df_conserv.consOnlyNPS==1);
