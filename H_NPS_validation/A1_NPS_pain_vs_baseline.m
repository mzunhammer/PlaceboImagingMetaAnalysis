...
stats.NPS(i)=withinMetastats(nanmean([control,placebo],2),0);
...
stats.NPS(i)=withinMetastats(nanmean([control,placebo],2),0);
stats.NPS(i)=withinMetastats(nanmean([con_rating,pla_rating],2),0);
...
%control_neg_rating=df((strcmp(df.studyID,'kessner')&strcmp(df.cond,'pain_control_neg')),'rating');
placebo_neg_rating=df((strcmp(df.studyID,'kessner')&strcmp(df.cond,'pain_placebo_neg')),'rating');
%control_pos_rating=df((strcmp(df.studyID,'kessner')&strcmp(df.cond,'pain_control_pos')),'rating');
placebo_pos_rating=df((strcmp(df.studyID,'kessner')&strcmp(df.cond,'pain_placebo_pos')),'rating');

i=find(strcmp(studies,'kessner'));
% Get stats.NPS for between-subject design
...
stats.NPS(i)=withinMetastats([placebo_neg{:,:};placebo_pos{:,:}],0);
stats.rating(i)=withinMetastats([placebo_neg_rating{:,:};placebo_pos_rating{:,:}],0);
...
% Get stats.NPS for between-subject design
stats.NPS(i)=withinMetastats([control{:,:};placebo{:,:}],0);
stats.rating(i)=withinMetastats([con_rating{:,:};pla_rating{:,:}],0);
...
stats.NPS(i)=withinMetastats(pain_effect,0);
stats.rating(i)=withinMetastats(rating_diff,0);
...
NPS_pos_imgs=vertcat(stats.NPS.delta)>0;
