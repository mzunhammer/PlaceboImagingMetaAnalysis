...
%
% uniform: Indicate whether the probability of the effect under H0 is distributed
% uniformly (1), or normally (0).
% Choose "uniform" (1) for a H0, when all values in a certain range (between an upper and an lower bound) are
% equally plausible.
% Choose "normal" (0) when values near a certain value
% (often the population mean) are more likely than values far from that value.
% >> population mean and SD of mean (often the population "standard error")
% are required.
% 
% 'bounds' (required if uniform = 1)
%  enter the lower and upper bound of uniformly distributed H0 as a two-element vector, e.g.:
%  >'bounds', [0,1]
% for a H0 where all effects between 0 and 1 are equally plausible.
%
% 'popnorm'
%Required if uniform = 0
% enter the population mean effect, the SD of population effect (SE) and
% the number of tails under H0, as a three-element vector e.g:
% [0,1,1] for a H0 where there is no population mean effect, an SD of 1 and
% effects are only expected in one direction.
% OR
% [1,0.5,2] for a H0 where there is a population mean effect of 1 is expected, with a SD of 0.5 and
...
if nargin>4
   disp('Too many input arguments. EITHER bounds OR popnorm required as additional argument, not both.');
   return
end
