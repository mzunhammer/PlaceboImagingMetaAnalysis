...
set(ax,'box','off',...
 ...
 'FontName',font_name)
 xlabel([outcomelabel,' with 95% CI; IV, random'],'Interpreter','latex');
 yscale=diff(get(ax,'Ylim'));
 xscale=diff(get(ax,'Xlim'));
 xy_rel=(xscale)/(yscale); %get relative size of axis in axis space, adding one to each, since we are counting the steps on the scale, not the difference of maxima!
...
box_scaling=1;   % basic unit is y (height of a study-line)
...
txt_position_eff=x_axis_size+x_axis_size*0.05;
txt_position_n=x_axis_size+x_axis_size;
