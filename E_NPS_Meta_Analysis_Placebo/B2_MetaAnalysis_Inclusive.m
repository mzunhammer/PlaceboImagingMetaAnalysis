...
v=find(strcmp(df_incl.variables,'rating'));
for i=1:length(df_incl.studies) % Calculate for all studies except...
    if df_incl.consOnlyRating(i)==0 %...data-sets where both pla and con is available
        if df_incl.BetweenSubject(i)==0 %Within-subject studies
           stats.rating(i)=withinMetastats(df_incl.pladata{i}(:,v),df_incl.condata{i}(:,v));
        elseif df_incl.BetweenSubject(i)==1 %Between-subject studies
           stats.rating(i)=betweenMetastats(df_incl.pladata{i}(:,v),df_incl.condata{i}(:,v));
        end        
    end
end
% Calculate for those studies where only pla>con contrasts are available
conOnly=find(df_incl.consOnlyRating==1);
impu_r=nanmean([stats.rating.r]); % impute the mean within-subject study correlation observed in all other studies
for i=conOnly'
stats.rating(i)=withinMetastats(df_incl.pladata{i}(:,v),impu_r);
end

%% Meta-Analysis NPS
v=find(strcmp(df_incl.variables,'NPSraw'));
for i=1:length(df_incl.studies) % Calculate for all studies except...
    if df_incl.consOnlyNPS(i)==0 %...data-sets where both pla and con is available
        if df_incl.BetweenSubject(i)==0 %Within-subject studies
           stats.NPS(i)=withinMetastats(df_incl.pladata{i}(:,v),df_incl.condata{i}(:,v));
        elseif df_incl.BetweenSubject(i)==1 %Between-subject studies
           stats.NPS(i)=betweenMetastats(df_incl.pladata{i}(:,v),df_incl.condata{i}(:,v));
        end        
    end
end
% Calculate for those studies where only pla>con contrasts are available
conOnly=find(df_incl.consOnlyNPS==1);
impu_r=nanmean([stats.NPS.r]); % impute the mean within-subject study correlation observed in all other studies
for i=conOnly'
stats.NPS(i)=withinMetastats(df_incl.pladata{i}(:,v),impu_r);
end

%% Meta-Analysis SIIPS
v=find(strcmp(df_incl.variables,'SIIPS'));
for i=1:length(df_incl.studies) % Calculate for all studies except...
    if df_incl.consOnlyNPS(i)==0 %...data-sets where both pla and con is available
        if df_incl.BetweenSubject(i)==0 %Within-subject studies
           stats.SIIPS(i)=withinMetastats(df_incl.pladata{i}(:,v),df_incl.condata{i}(:,v));
        elseif df_incl.BetweenSubject(i)==1 %Between-subject studies
           stats.SIIPS(i)=betweenMetastats(df_incl.pladata{i}(:,v),df_incl.condata{i}(:,v));
        end        
    end
end
% Calculate for those studies where only pla>con contrasts are available
...
for i=conOnly'
stats.SIIPS(i)=withinMetastats(df_incl.pladata{i}(:,v),impu_r);
end
...
for i = 1:numel(varnames)
    ...
    crop(fullfile(pubpath,['B2_Meta_Incl_',varnames{i},'.png']));
