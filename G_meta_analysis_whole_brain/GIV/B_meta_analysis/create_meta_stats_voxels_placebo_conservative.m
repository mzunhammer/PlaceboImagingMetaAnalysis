function stats=create_meta_stats_voxels_placebo_conservative(df,dfv)
...
for i=1:size(df,1) % Calculate for all studies except...
    if ~df.excluded_conservative_sample(i) && df.contrast_imgs_only(i)==0 %...data-sets where only contrasts are available
        if strcmp(df.study_design{i}, 'within') %Calculate within-subject studies
           ex=df.subjects{i}.excluded;
           stats(i)=summarize_within(dfv.pain_placebo{i}(~ex,:),dfv.pain_control{i}(~ex,:));
        elseif strcmp(df.study_design{i}, 'between') %Calculate between-group studies
           stats(i)=summarize_between(dfv.pain_placebo{i},dfv.pain_control{i}); % no subjects had to be excluded for between-group studies
        end        
    end
end
...
for i=conOnly'
    if ~df.excluded_conservative_sample(i)
    ex=df.subjects{i}.excluded;
    stats(i)=summarize_within(dfv.placebo_minus_control{i}(~ex,:),impu_r); % THE con_img VECTOR INCLUDES PAIN vs BASELINE CONTRAST AND IS NOT TO BE USED USED IN PLACEBO VS CONTROL ANALYSIS
    end
end
...
for i=1:length(stats)
    ...
    if ~isempty(stats(i).delta) % necessary as "sum" returns 0 for [] for some stupid reason
        stats(i).r_external=fastcorrcoef(stats(i).delta,df.GIV_stats_rating(i).delta(~ex,:),'exclude_nan'); % correlate single-subject effect of behavior and voxel signal 
        stats(i).n_r_external=sum(~(isnan(stats(i).delta)|... % the n for the correlation is the n of subjects showing non-nan values at that particular voxels
                                         isnan(df.GIV_stats_rating(i).delta(~ex,:)))); % AND non nan-ratings
    else
